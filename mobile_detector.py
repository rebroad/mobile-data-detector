...
class MobileDetector:
    ...
    def is_android_connection(self) -> bool:
        ...
        try:
                ...
                for line in devs_out.strip().split('\n'):
                    ...
                    if dtype == 'wifi' and state and state.startswith('connected'):
                        if default_iface is None or device == default_iface:
                            if connection_name:
                                # If SSID is known mobile, confirm Android/mobile tether
                                for allowed in self.config.get('android_ssid_whitelist', []):
                                    if connection_name == allowed:
                                ...
                                for unthrottled in self.config.get('unthrottled_ssid_whitelist', []):
                                    if connection_name == unthrottled:
            ...
            # 4) Fallback to public IP carrier check
            if self.is_mobile_data():
                logger.info("Android/mobile tether inferred from mobile carrier public IP")
                return True
    ...
    def detect_mobile_data(self, force_android: Optional[bool] = None) -> bool:
        ...
        if rc == 0:
            for line in devs_out.strip().split('\n'):
                ...
                device, dtype, state, connection_name = dparts[0], dparts[1], dparts[2], dparts[3]
                if dtype == 'wifi' and state and state.startswith('connected') and connection_name:
                    if connection_name in self.config.get('android_ssid_whitelist', []) or \
                       connection_name in self.config.get('unthrottled_ssid_whitelist', []):
                        return True
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "test":
            ...
            if usage_stats:
                print(f"Usage: {usage_stats['total_gb']}/{usage_stats['allowance_gb']}GB ({usage_stats['remaining_percent']}% remaining)")
            ...
            print("Usage: python3 mobile_detector.py {test|status|start}")
    else:
        print("Usage: python3 mobile_detector.py {test|status|start}")
